# 【最新】【重要・必読】令和最新版ニュース記事の作成手順マニュアル（仮）_令和7年度第4四半期（1月対応）暫定最新決定版（2）のコピー更新版（開発共有用）_latest（修正版）v4.1_final_02
...
- `published` が `false` になっているとニュース記事にアクセスできなくなる。
...
メンバー追加を伴う記事追加の為に、Visual Studio Code の GitHub Copilot Chat の [Prompt file](https://docs.github.com/copilot/tutorials/customization-library/prompt-files)（カスタムスラッシュコマンド）
を用意しています:

```
/add-members-and-news
```
